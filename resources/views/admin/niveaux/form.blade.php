    ...
    <div id="formu" class="container">
            <div class="row">
                <div class="col-lg-6 offset-lg-3 my-5">
                    ...
                    <h3>@yield('title') :</h3>
                    <br>
                    <form class="vstack gap-5" action="{{ route($niveau->exists ? 'admin.niveau.update' : 'admin.niveau.store', $niveau) }}"
                        method="post">
                        @csrf

                        @method($niveau->exists ? 'put' : 'post')

                        @include('shared.input', ['name' => 'nom_niveau', 'label' => 'Nom :', 'placeholder' => 'Entrer le nom', 'value' => $niveau->nom_niveau])
